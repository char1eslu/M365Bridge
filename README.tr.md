...
Tüm model seçimi, M365 backend'ine gönderilen `tone` alanı ile yapılır. Tüm modeller için `Override` alanı boştur. Tüm model tanımlayıcıları aynı GPT-5 backend'ine yönlendirilir; `tone` alanı yanıt davranışını kontrol eder.

| Anahtar            | Tone              | OpenAI ID         | Düşünme? |
|--------------------|-------------------|-------------------|----------|
| `auto`             | Magic             | gpt-4-auto        | Hayır    |
| `quick`            | Chat              | gpt-4-quick       | Hayır    |
| `reasoning`        | Magic             | gpt-4-reasoning   | Hayır    |
| `gpt5.5`           | Gpt_5_5_Chat      | gpt-5.5           | Hayır    |
| `gpt5.5-reasoning` | Gpt_5_5_Reasoning | gpt-5.5-reasoning | Evet     |
...
`gpt5.5-reasoning`, modelin düşünme sürecini içeren `reasoning_content` çıktısı üretir. OpenAI endpoint'leri bunu `reasoning_content` olarak; Anthropic endpoint'leri `text` bloğundan önce bir `thinking` içerik bloğu olarak gösterir.
